"""
bitcoin-wallet/wallet.py
========================
From-scratch BIP-39/32/44 HD wallet implementation.
...
Key and address helpers (get_private_key, get_public_key, make_p2pkh_address)
...
from core.crypto import get_private_key, get_public_key, make_p2pkh_address
...
def _derive_btc_bip44_addresses(
    btc_master_priv: bytes,
    btc_master_chain_code: bytes,
    count: int
) -> list[list[bytes]]:
    """Derive `count` Bitcoin child keys via BIP-44 path: m/44'/0'/0'/0/i.
    ...
    priv_44, chain_44 = _get_child_key_hardened(
        btc_master_priv, btc_master_chain_code, 44 + HARDENED_OFFSET
    )
    # m / 44' / 0'
    priv_44_0, chain_44_0 = _get_child_key_hardened(
        priv_44, chain_44, 0 + HARDENED_OFFSET
    )
    # m / 44' / 0' / 0'
    priv_44_0_0, chain_44_0_0 = _get_child_key_hardened(
        priv_44_0, chain_44_0, 0 + HARDENED_OFFSET
    )
...
def main():
    ...
    btc_addresses = bip44.derive_bitcoin_addresses(
        btc_master_priv, btc_master_chain_code, count=5
    )
